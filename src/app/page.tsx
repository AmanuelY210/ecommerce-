      ...
      <div className="max-w-[1500px] mx-auto px-2 sm:px-4 py-4 sm:py-6 w-full">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-10 gap-3">
            {(loading ? Array.from({length: 10}) : categories).map((c, i) => (
              loading ? <Skeleton key={i} className="aspect-square rounded-lg" /> :
        ...
        <section className="mb-8">
          <div className="bg-gradient-to-r from-red-50 to-amber-50 rounded-lg border border-red-100 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {(loading ? Array.from({length: 6}) : deals.slice(0, 6)).map((p, i) => (
                loading ? <Skeleton key={i} className="aspect-[3/4] rounded-lg" /> :
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {(loading ? Array.from({length: 12}) : featured.slice(0, 12)).map((p, i) => (
              loading ? <Skeleton key={i} className="aspect-[3/4] rounded-lg" /> :
              <ProductCard key={p.id} product={p} onAdd={() => handleAddToCart(p)} />
            ))}
          </div>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {(loading ? Array.from({length: 8}) : vendors.slice(0, 8)).map((v, i) => (
              loading ? <Skeleton key={i} className="h-32 rounded-lg" /> :
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {(loading ? Array.from({length: 12}) : newArrivals.slice(0, 12)).map((p, i) => (
              loading ? <Skeleton key={i} className="aspect-[3/4] rounded-lg" /> :
              <ProductCard key={p.id} product={p} onAdd={() => handleAddToCart(p)} />
            ))}
          </div>
